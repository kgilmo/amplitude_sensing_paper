...
files_to_use = [0,1,2,3,5,6,7,8,9,10,11,12,4]
...
for i,fn in enumerate(fns):
    ...
    bp_bcks += [np.array([val for sublist in np.delete(bp,1,axis=2) for val in sublist]).flatten()]
    bp_sigs += [np.array([val for sublist in np.delete(bp,0,axis=2) for val in sublist]).flatten()]
...
for j,amp in enumerate(amps):
    if j is 2:
        bp_sig = np.concatenate((bp_sigs[2],bp_sigs[-1]))
        bp_bck = np.concatenate((bp_bcks[2],bp_bcks[-1]))
    else:    
        bp_sig = bp_sigs[j]
        bp_bck = bp_bcks[j]
...
#%%        
plt.errorbar(amps,SNs,yerr=errs,linestyle = '', marker='o',label='S/N using std dev of diffs')
